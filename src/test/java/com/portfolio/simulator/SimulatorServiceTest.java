...
class SimulatorServiceTest {
    ...
    void annuityCompare_deferredIncomeStart_boostsAnnuityRate() {
        // Income Start Year 7 = 6 years of deferral past purchase (year 1). Base rate
        // and the increase % are both looked up at the CURRENT age (65); the bonus is
        // that increase % multiplied flatly by the number of deferral years.
        AnnuityCompareRequest req = new AnnuityCompareRequest();
        req.setAge(65);
        req.setJoint(false);
        ...
        assertEquals(expectedRate, resp.getAnnuityRate(), 0.0001,
            "Deferred annuity rate should use the current-age base rate plus (current-age increase % × deferral years)");
    }
